  ...
  shiny::tagList(
    ...
    shiny::fluidRow(
      ...
      shiny::column(
        ...
        # Create single omics
        bs4Dash::box(
          id = ns('box_create_single_omics'),
          title = "Create single omics",
          width = 12,
          collapsible = F,
          solidHeader = T,
          status = "gray",
  ...
  shiny::moduleServer(
    ...
    function(input, output, session) {
      ...
      shiny::observeEvent(input$start_method, {
        if (input$start_method == "New") {
          if (input$box_create_single_omics$collapsed) {
            bs4Dash::updateBox(
              id = 'box_create_single_omics',
              action = "toggle",
              session = session
            )
          }
